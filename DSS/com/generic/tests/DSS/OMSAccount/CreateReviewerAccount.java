...
//DSS
public class CreateReviewerAccount extends SelTestCase {

	public static void startTest(String ReviewerMail, LinkedHashMap<String, String> userdetails) throws Exception {
		...
		try {
			...
			Account.typeEmailAddress(ReviewerMail);
...
//			Account.selectAllowedChangeToBillingAddress();
			...
			for (int index = 3; index < 6 * newNumOfUsersAvailable; index = index + 6) {
				if (Account.getUsersTableData(index).contains(ReviewerMail)) {
					logs.debug("<font color=#33BEFF>" + ReviewerMail + " User Added Successfully </font>");
					break;
				}
			}
			int userIndex = Account.getUserIndex(ReviewerMail,newNumOfUsersAvailable );
